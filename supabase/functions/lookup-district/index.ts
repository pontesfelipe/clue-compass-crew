...
serve(async (req) => {
  ...
  try {
    ...
    if (!zipCode || zipCode.length < 5) {
      return new Response(
        JSON.stringify({ error: "Valid ZIP code required" }),
    ...
    // Use Google Civic Information API (free, no API key required for basic usage)
    const civicUrl = `https://www.googleapis.com/civicinfo/v2/representatives?address=${cleanZip}&levels=country&roles=legislatorLowerBody&roles=legislatorUpperBody&key=`;
    
    // Try with Census Bureau Geocoding API first (completely free, no key)
    const censusUrl = `https://geocoding.geo.census.gov/geocoder/geographies/onelineaddress?address=${cleanZip}&benchmark=Public_AR_Current&vintage=Current_Current&layers=54&format=json`;
    
    console.log("Looking up district for ZIP:", cleanZip);
    
    const censusResponse = await fetch(censusUrl);
    
    if (censusResponse.ok) {
      const censusData = await censusResponse.json();
      console.log("Census response:", JSON.stringify(censusData));
      
      // Extract congressional district from Census response
      const geographies = censusData?.result?.geographies;
      const congressionalDistricts = geographies?.["Congressional Districts"];
      
      if (congressionalDistricts && congressionalDistricts.length > 0) {
        const district = congressionalDistricts[0];
        const cdCode = district.CD119 || district.CD || district.BASENAME;
        ...
        const stateAbbr = stateFips[stateCode];
        
        if (stateAbbr && cdCode) {
          // Remove leading zeros from district number
          const districtNum = String(parseInt(cdCode, 10));
          
          console.log(`Found district: ${stateAbbr}-${districtNum}`);
    ...
    // Fallback: derive state from ZIP prefix (we can at least get senators)
    ...
    const stateFromZip = zipToState[zipPrefix];
    if (stateFromZip) {
      console.log(`Could not find district, but found state: ${stateFromZip}`);
      return new Response(
        JSON.stringify({ 
          state: stateFromZip, 
          district: null,
    ...
    return new Response(
      JSON.stringify({ error: "Could not determine district for ZIP code" }),
